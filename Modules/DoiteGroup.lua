...
local function _DG_UI_GetData(ctx)
  if not ctx then return nil end
  return ctx.api.Ensure(ctx.state.key)
end

local function _DG_UI_RefreshAll(ctx)
...
local function _DG_UI_HideAll(ctx)
  local w = ctx.w
  local i = 1
...
local function _DG_UI_SetAddState(ctx)
  local w = ctx.w
...
local function _DG_UI_Join(ctx, kind, name)
  local d = _DG_UI_GetData(ctx); if not d then return end
  local state = ctx.state
  if kind == "group" then
    d.category = nil
    d.group = name
  ...
  _DG_UI_RefreshAll(ctx)
end

local function _DG_UI_LeaveCurrent(ctx)
  local d = _DG_UI_GetData(ctx); if not d then return end
  local state = ctx.state
...
local function _DG_UI_RenameAll(ctx, newName)
  local d = _DG_UI_GetData(ctx); if not d then return end
  local state = ctx.state
...
local _DG_UI_Refresh

local function _DG_UI_InitExistingDD(ctx)
  local w = ctx.w
  UIDropDownMenu_Initialize(w.groupDD, function()
    ...
    while i <= table.getn(arr) do
      ...
      info.text = n
      info.value = n
      info.func = function() _DG_UI_Join(ctx, "group", n); _DG_UI_Refresh(ctx) end
  ...
  UIDropDownMenu_Initialize(w.catDD, function()
    ...
    while i <= table.getn(arr) do
      ...
      info.text = n
      info.value = n
      info.func = function() _DG_UI_Join(ctx, "category", n); _DG_UI_Refresh(ctx) end
      UIDropDownMenu_AddButton(info)
      i = i + 1
    end
  end)
...
_DG_UI_Refresh = function(ctx)
  local d = _DG_UI_GetData(ctx)
...
local function _DG_UI_WireHandlers(ctx)
  local w = ctx.w
  local state = ctx.state

  w.bNew:SetScript("OnClick", function() state.step = "newkind"; _DG_UI_Refresh(ctx) end)
  w.bExisting:SetScript("OnClick", function() state.step = "existing"; _DG_UI_Refresh(ctx) end)
  w.bGroup:SetScript("OnClick", function() state.mode = "group"; state.step = "newname"; state.rename = false; state.renameFrom = nil; w.nameIn:SetText(""); _DG_UI_Refresh(ctx) end)
  w.bCat:SetScript("OnClick", function() state.mode = "category"; state.step = "newname"; state.rename = false; state.renameFrom = nil; w.nameIn:SetText(""); _DG_UI_Refresh(ctx) end)
  w.bBackA:SetScript("OnClick", function() state.step = "pick"; _DG_UI_Refresh(ctx) end)
  w.bBackB:SetScript("OnClick", function()
    if state.rename then
      state.step = (state.mode == "group") and "ingroup" or "incategory"
    else
      state.step = "newkind"
    end
    _DG_UI_Refresh(ctx)
  ...
  w.bRename:SetScript("OnClick", function()
    local d = _DG_UI_GetData(ctx)
    state.rename = true
    state.renameFrom = (state.mode == "group") and d.group or d.category
    w.nameIn:SetText(state.renameFrom or "")
    state.step = "newname"
    _DG_UI_Refresh(ctx)
  end)
  w.bLeave:SetScript("OnClick", function() state.step = "confirmleave"; _DG_UI_Refresh(ctx) end)
  w.bYes:SetScript("OnClick", function() _DG_UI_LeaveCurrent(ctx); _DG_UI_Refresh(ctx) end)
  w.bBackC:SetScript("OnClick", function() state.step = (state.mode == "group") and "ingroup" or "incategory"; _DG_UI_Refresh(ctx) end)
  w.bBackD:SetScript("OnClick", function() state.step = "pick"; _DG_UI_Refresh(ctx) end)
  w.bSettings:SetScript("OnClick", function() state.step = "settings"; _DG_UI_Refresh(ctx) end)
  w.bBackSettings:SetScript("OnClick", function() state.step = "ingroup"; _DG_UI_Refresh(ctx) end)
  ...
  w.leaderCB:SetScript("OnClick", function(self)
    ...
    if self:GetChecked() then
      for k, s in pairs(_DA_DB().spells) do
        if s.group == d.group and k ~= state.key then
          s.isLeader = false
        end
      end
      d.isLeader = true
  ...
  w.nameIn:SetScript("OnTextChanged", function() _DG_UI_SetAddState(ctx) end)
  w.bAdd:SetScript("OnClick", function()
    local picked = _TrimName(w.nameIn:GetText() or "")
    if picked == "" then return end
    if state.rename then _DG_UI_RenameAll(ctx, picked) else _DG_UI_Join(ctx, state.mode, picked) end
    w.nameIn:SetText("")
    _DG_UI_Refresh(ctx)
  end)
end
